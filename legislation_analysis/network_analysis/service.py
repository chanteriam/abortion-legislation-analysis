...
def run_network_analysis() -> None:
    ...
    legislation_network = NetworkAnalysis(
        congress_file=os.path.join(PROCESSED_DATA_PATH, CONGRESS_DATA_FILE_NER_NAME),
        scotus_file=os.path.join(PROCESSED_DATA_PATH, SCOTUS_DATA_FILE_NER_NAME),
    )
    ...
    congr_scotus_ref_matrix = pd.DataFrame(legislation_network.congress_scotus_refs)
    ...
    save_df_to_file(
        congr_ref_matrix,
        os.path.join(NETWORK_DATA_PATH, "congress_congress_references_matrix.fea"),
    )

    save_df_to_file(
        scotus_ref_matrix,
        os.path.join(NETWORK_DATA_PATH, "congress_scotus_references_matrix.fea"),
    )
