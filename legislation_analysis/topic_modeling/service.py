...
def run_topic_modeling() -> None:
    ...
    congress_model_fp = (
        os.path.join(MODELED_DATA_PATH, CONGRESS_TOPIC_MODEL_FILE_NAME)
        if os.path.exists(
            os.path.join(MODELED_DATA_PATH, CONGRESS_TOPIC_MODEL_FILE_NAME)
        )
        else None
...
def run_dynamic_topic_modeling() -> None:
    ...
    model_fp = (
        os.path.join(MODELED_DATA_PATH, f"{CONGRESS_DYN_TOPIC_MODEL_FILE_NAME}")
        if os.path.exists(
            os.path.join(
                MODELED_DATA_PATH, f"{CONGRESS_DYN_TOPIC_MODEL_FILE_NAME}"
            )
        )
        else None
    )
